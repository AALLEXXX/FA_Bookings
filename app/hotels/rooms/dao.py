            ...
            stmt = (
                select(
                    Hotels.id.label('hotel_id'),
                       ...
                       (Rooms.quantity - func.coalesce(func.count(Bookings.id), 0))
                       .label('rooms_left'))
                       .select_from(Hotels)
                       .join(Rooms, Rooms.hotel_id == hotel_id, isouter=True)
                       .join(Bookings, (Rooms.id == Bookings.room_id) & 
                             ((Bookings.date_to <= date_to) & (Bookings.date_from >= date_from)), isouter=True
                             )
                             .filter(Hotels.id == hotel_id)
                             .group_by(Hotels.id, Rooms.name,
                                        Rooms.price, Rooms.description,
